...
const getters = {
	...
	getClientQrcode: (state) => (id) => {
		const item = state.clientQrcodes.find(item => item.id === id)
	...
	getClientConfig: (state) => (id) => {
		const item = state.clientConfigs.find(item => item.id === id)
...
const actions = {
	...
	readQrcode({ state, commit }, client) {
		ApiService.getWithConfig(`/client/${client.id}/config?qrcode=true`, { responseType: 'arraybuffer' })
			.then(resp => {
				const image = Buffer.from(resp, 'binary').toString('base64')
...
const mutations = {
	...
	clientQrcodes(state, { client, image }) {
		const index = state.clientQrcodes.findIndex(x => x.id === client.id);
		if (index !== -1) {
	...
	clientConfigs(state, { client, config }) {
		const index = state.clientConfigs.findIndex(x => x.id === client.id);
		if (index !== -1) {
